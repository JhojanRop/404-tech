...
const navigation = {
  ...
  social: [
    {
      name: 'Instagram',
      href: 'https://www.instagram.com/p/CSeYforAqm4/',
    ...
    {
      name: 'GitHub',
      href: 'https://github.com/JhojanRop/404-tech',
      icon: (props: React.SVGProps<SVGSVGElement>) => (
        <svg fill="currentColor" viewBox="0 0 24 24" {...props}>
          <path
            fillRule="evenodd"
...
function ListItems({ data }: { data: { name: string, href: string }[] }) {
  return (
    <ul role="list" className="mt-6 space-y-4">
      {data.map(({ name, href }) => (
        <li key={name}>
          <Link href={href} className="text-sm/6 text-gray-600 underline hover:text-gray-900">
            {name}
          </Link>
        </li>
      ))}
      ...
      <div className="max-w-7xl w-11/12 h-full mx-auto flex flex-col justify-between gap-8">
        <div className="xl:grid xl:grid-cols-3 xl:gap-8">
          ...
          <div className="mt-16 grid grid-cols-2 gap-8 md:grid-cols-3 xl:col-span-2 xl:mt-0">
            <div>
              <h3 className="text-sm/6 font-semibold text-gray-900">Customer Services</h3>
              <ListItems data={navigation.customerServices} />
            </div>

            <div>
              <h3 className="text-sm/6 font-semibold text-gray-900">About Us</h3>
              <ListItems data={navigation.aboutUs} />
            </div>

            <div>
              <h3 className="text-sm/6 font-semibold text-gray-900">Categories</h3>
              <ListItems data={navigation.categories} />
            </div>
          </div>
        </div>
